...
# Usage: collect.sh <binary>   (a build where FD_RAW=1 skips sorting and --top output truncation)
# Output: build/raw/<run>/<query>.tsv
set -uo pipefail
source "$(dirname "$0")/../../scripts/lib.sh"
D=$(cd "$(dirname "$0")/.." && pwd)
export XBIN=$1 FD_RAW=1 RAW=$D/build/raw IDX_MCSA IDX_HSAPIENS REPO
...
MUT=$BENCH/build/mutant_250/spec.tsv
SPEC=$MCSA_DIR/queryspec_250.tsv
...
motif_run mo_pm_def   --format-output $PM
motif_run mo_pm_sens  --format-output $PM --sensitive
motif_run mo_ps_def   --per-structure --format-output $PS
motif_run mo_ps_skip  --skip-match --format-output $PS
...
mcsa_run mc_pm_blosum $SPEC orig --format-output $PM --aa-subst blosum62
mcsa_run mc_ps_skip  $SPEC orig --skip-match --format-output $PS
echo done
